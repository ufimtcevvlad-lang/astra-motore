...
export default function ExcelImport() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setPreview(data);
      // Default: all duplicates set to skip
      const flags: Record<number, boolean> = {};
      data.duplicates.forEach((_: DuplicateItem, i: number) => { flags[i] = false; });
  ...
  const newCount = preview?.newItems.length ?? 0;
  ...
  const handleConfirm = async () => {
    ...
    setError(null);
    try {
      const updateItems = preview.duplicates
        .filter((_, i) => updateFlags[i])
        .map((d) => ({
          id: d.existing.id,
          name: d.parsed.name,
          sku: d.parsed.sku,
          brand: d.parsed.brand,
          price: d.parsed.price,
        }));
      const res = await fetch("/api/admin/products/import/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ newItems: preview.newItems, updateIds: updateItems }),
      });
      const data = await res.json();
      if (!res.ok) {
    ...
    return (
      <div className="space-y-6 mt-6">
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="bg-green-50 px-6 py-3 border-b border-green-100">
              <h3 className="text-sm font-semibold text-green-700">
                Новые товары ({preview.newItems.length})
              </h3>
            </div>
                ...
                <tbody>
                  {preview.newItems.map((item, i) => (
                    <tr key={i} className="border-b border-gray-100">
                      ...
                      <td className="px-6 py-3 text-right">{formatPrice(item.price)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {preview.duplicates.map((dup, i) => {
                    const nameChanged = dup.existing.name !== dup.parsed.name;
                    const priceChanged = dup.existing.price !== dup.parsed.price;
                    return (
                      <tr key={i} className="border-b border-gray-100">
                        <td className="px-6 py-3 font-mono text-xs">{dup.parsed.sku}</td>
                        <td className="px-6 py-3">
                          <div className="text-gray-500 text-xs line-through">{dup.existing.name}</div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 flex items-center justify-between">
          <p className="text-sm text-gray-600">
            Будет добавлено: <span className="font-semibold text-gray-900">{newCount}</span>
            {" | "}Обновлено: <span className="font-semibold text-gray-900">{updateCount}</span>
            {" | "}Пропущено: <span className="font-semibold text-gray-900">{skipCount}</span>
          </p>
    ...
    return (
      <div className="max-w-lg mx-auto mt-8">
        <div className="bg-white rounded-xl shadow-sm p-6 text-center">
          ...
          <p className="text-sm text-gray-600">
            Добавлено: {result.added}, обновлено: {result.updated}, ошибок: {result.errors.length}
          </p>
          {hasErrors && (

...
/**
 * Расширенное описание товара: функция в автомобиле, признаки износа,
...
/** Структурированная запись применимости к автомобилю. */
export type ProductFitment = {
  /** Марка авто (Opel, Chevrolet, …) */
  brand: string;
  /** Модель (Astra H, Cruze, Insignia, …) */
  model: string;
  /** Годы выпуска, напр. "2004–2014". */
  years?: string;
  /** Двигатель, напр. "1.6 Z16XER" или "1.8 бензин". */
  engine?: string;
  /** Кузов: седан, хэтчбек, универсал и т.п. */
  body?: string;
  /** Дополнительное уточнение (комплектация, исполнение и пр.) */
  note?: string;
};

/** Кросс-номер / OEM-аналог от другого производителя. */
export type ProductCrossNumber = {
  /** Бренд-производитель, напр. "Opel", "GM", "Bosch". */
  brand: string;
  /** Артикул у этого бренда. */
  sku: string;
};
...
export type Product = {
  ...
  specs?: ProductSpec[];
  /** Расширенное описание с разбивкой по разделам. */
  longDescription?: ProductLongDescription;
  /** Структурированная применимость к авто (заменяет/дополняет поле `car`). */
  fitment?: ProductFitment[];
  /** Кросс-номера / OEM-аналоги (артикулы этой детали у других брендов). */
...
const OPEL_PILOT_RAW: Array<{
...
}> = [
  {
    ...
    specs: [
      { label: "Тип", value: "Свеча зажигания искровая" },
      { label: "Центральный электрод", value: "Иттрий, тонкий" },
      { label: "Боковой электрод", value: "1 шт., никелевый сплав" },
      { label: "Зазор", value: "0.9 мм" },
      { label: "Резьба", value: "M14 × 1.25" },
      { label: "Длина резьбовой части", value: "26.5 мм" },
      { label: "Размер под ключ", value: "16 мм" },
      { label: "Момент затяжки", value: "25–30 Н·м" },
      { label: "Калильное число (Bosch)", value: "6" },
      { label: "Количество в упаковке", value: "1 шт." },
    ],
    longDescription: {
      purpose:
        "Свеча зажигания поджигает топливо-воздушную смесь в цилиндре двигателя. От её состояния зависит запуск мотора, стабильность холостого хода, расход топлива и отсутствие пропусков воспламенения.",
      symptoms:
        "Затруднённый запуск, троение на холостых, рывки при разгоне, повышенный расход топлива, ошибки P0300–P0304 по пропускам зажигания, нагар на электродах или выгоревший центральный электрод.",
      interval:
        "Плановая замена — каждые 30 000 км пробега по регламенту Opel/Chevrolet для этого семейства моторов (или раньше при использовании некачественного топлива). Меняются комплектом из 4 шт.",
      install:
        "Менять на холодном двигателе. Момент затяжки — 25–30 Н·м, перетяжка повреждает резьбу в ГБЦ. Резьбу новых свечей смазывать не нужно — заводское покрытие уже предусмотрено.",
    },
    fitment: [
      {
        brand: "Opel",
        model: "Astra H",
        years: "2004–2014",
        engine: "1.6 Z16XEP / Z16XER",
        body: "Седан, хэтчбек, универсал, GTC",
      },
      {
        brand: "Opel",
        model: "Astra J",
        years: "2009–2015",
        engine: "1.6 A16XER, 1.8 A18XER",
        body: "Седан, хэтчбек, универсал",
      },
      {
        brand: "Opel",
        model: "Zafira B",
        years: "2005–2014",
        engine: "1.6 Z16XEP, 1.8 Z18XE / Z18XER",
        body: "Компактвэн",
      },
      {
        brand: "Opel",
        model: "Meriva B",
        years: "2010–2017",
        engine: "1.4 A14XER / A14NEL, 1.8 Z18XER",
      },
      {
        brand: "Opel",
        model: "Insignia A",
      ...
      },
      {
        ...
        model: "Cruze",
        years: "2009–2015",
        engine: "1.6 F16D3, 1.8 F18D4",
        note: "Уточнить индекс двигателя по VIN перед заказом.",
      },
    ],
    crossNumbers: [
      { brand: "Bosch", sku: "0 242 229 699" },
      { brand: "Bosch", sku: "FR7DPP33U" },
      { brand: "GM OE", sku: "12622561" },
      { brand: "GM OE", sku: "55562243" },
      { brand: "ACDelco", sku: "41-109" },
      { brand: "NGK", sku: "ZFR6U9" },
      { brand: "Denso", sku: "IKH20" },
    ],
  },
  {
    id: "opel-2",
    ...
    specs: [
      { label: "Тип", value: "Форсунка охлаждения поршня" },
      { label: "Расположение", value: "Блок цилиндров, под поршнем" },
      { label: "Материал корпуса", value: "Сталь" },
      { label: "Принцип работы", value: "Масляный клапан с шариковым запором" },
      { label: "Давление открытия", value: "~1.5–2.0 бар" },
      { label: "Количество на двигатель", value: "4 шт. (по одной на цилиндр)" },
      { label: "Производитель", value: "GM OE (оригинал)" },
    ],
    longDescription: {
      purpose:
        "Форсунка подаёт струю моторного масла на внутреннюю поверхность днища поршня. Это охлаждает самую горячую зону поршневой группы, снижает риск детонации и продлевает ресурс колец. На моторах Ecotec с прямым впрыском форсунки играют особенно важную роль — без них поршень перегревается под нагрузкой.",
      symptoms:
        "Стуки в районе блока на прогретом моторе под нагрузкой, повышенный расход масла, детонация на высоких оборотах, подтеки масла в зоне блока после разборки. Забитые или текущие форсунки приводят к задирам поршней — ремонт очень дорогой.",
      interval:
        "По регламенту не меняется. Замена — только при капитальном ремонте двигателя или при обнаружении течи/закоксовки. Рекомендуется менять весь комплект (4 шт.) одновременно.",
      install:
        "Замена выполняется при снятом поддоне картера. Проверить чистоту масляных каналов блока, использовать новое уплотнение. После установки обязательна проверка давления масла на прогретом двигателе.",
    },
    fitment: [
      {
        brand: "Opel",
        model: "Astra H / J",
        years: "2004–2015",
        engine: "1.6 Z16XER, A16XER",
        note: "Моторы с фазовращателями и прямым впрыском",
      },
      {
        brand: "Opel",
        model: "Zafira B / C",
      ...
      },
      {
        ...
        years: "2009–2015",
        engine: "1.6 F16D4",
        note: "Применимость уточнять по VIN.",
      },
    ],
    crossNumbers: [
      { brand: "GM OE", sku: "55564441" },
      { brand: "Opel", sku: "5 55 644" },
      { brand: "Febi Bilstein", sku: "101358" },
    ],
  },
  {
    id: "opel-3",

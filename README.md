...
- Разделы каталога на главной: `src/app/data/catalog-sections.ts` — группы `CATALOG_GROUPS` (крупные блоки) и подразделы `CATALOG_SECTIONS`; у товара поле `category` совпадает с `title` раздела.
